...
function searchTopics(query) {
  ...
  for (const key of Object.keys(TOPIC_ANSWERS)) {
    ...
    const matchCount = keyWords.filter(
      (kw) => queryWords.some((qw) => qw.includes(kw) || kw.includes(qw))
    ).length;
  ...
  if (q.length >= 3) {
    for (const key of Object.keys(TOPIC_ANSWERS)) {
      ...
      for (const word of words) {
        if (word.length >= 4 && (key.includes(word) || TOPIC_ANSWERS[key].title.toLowerCase().includes(word) || TOPIC_ANSWERS[key].subject.toLowerCase().includes(word))) {
          return TOPIC_ANSWERS[key];
        }
      }
    }
