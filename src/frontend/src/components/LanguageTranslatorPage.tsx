...
import { ChevronRight, Copy, Languages, RotateCcw } from "lucide-react";
...
type TranslationMap = Partial<Record<LangCode, string>>;

const WORDS: Record<string, TranslationMap> = {
  // Greetings & basics
  hello: {
    hi: "नमस्ते",
    ta: "வணக்கம்",
...
export default function LanguageTranslatorPage() {
  ...
  return (
    <section className="container mx-auto px-4 py-10 max-w-3xl">
      ...
      <div className="mb-8 rounded-2xl overflow-hidden bg-gradient-to-br from-[oklch(0.52_0.18_335)] to-[oklch(0.36_0.14_310)] p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 text-white/60 text-xs font-medium mb-3">
          <Languages className="w-3.5 h-3.5" />
          <span>Tools</span>
          <ChevronRight className="w-3 h-3" />
          <span className="text-white/90">Language Translator</span>
        </div>
        <h1 className="font-display font-bold text-3xl md:text-4xl text-white mb-2">
          Language Translator
        </h1>
        <p className="text-white/70 text-base">
          Translate English words and phrases to 10 Indian languages. Type words
          and choose your target language.
        </p>
        ...
        <div className="flex flex-wrap gap-2">
          {LANGUAGES.map((lang) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium border-2 transition-all duration-150 ${
            ...
            >
              <span>{lang.flag}</span>
              <span>{lang.name}</span>
              <span className="text-xs opacity-70">{lang.script}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      >
        ...
        Translate to {selectedLang.name} ({selectedLang.script})
      </Button>
        ...
        <div className="bg-card rounded-2xl border-2 border-primary/20 p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Badge className="text-sm font-semibold px-3 py-1">
                {selectedLang.flag} {selectedLang.name} — {selectedLang.script}
              </Badge>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="p-4 bg-secondary/40 rounded-xl">
            ...
            <p className="text-lg font-semibold text-foreground leading-relaxed">
              {result.map((r) => r.translation).join(" ")}
            </p>
          </div>

          {result.some((r) => r.translation.startsWith("[")) && (
            <p className="text-xs text-muted-foreground mt-3 italic">
              Words in [brackets] are not yet in our dictionary. We support 200+
              common words across education, nature, family, and everyday life.
            </p>
          )}
        </div>
      )}
